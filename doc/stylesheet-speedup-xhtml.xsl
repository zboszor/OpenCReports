          ...
          <xsl:choose>
            <xsl:when test="$index.links.to.section = 1">
              <xsl:call-template name="href.target">
                ...
                <!-- Optimization for ocrpt-docs: We only have an index as a
...
<xsl:template name="reference">
  ...
  <xsl:choose>
    <xsl:when test="contains($zones, ' ')">
      ...
      <a>
        <xsl:apply-templates select="." mode="class.attribute"/>
<!--    Optimization for ocrpt-docs: this call adds nothing but fails with docbook-xsl 1.76 -->
...
<!--    <xsl:call-template name="id.attribute"/> -->
        <xsl:attribute name="href">
          <xsl:call-template name="href.target">
            <xsl:with-param name="object" select="$target[1]"/>
            <!-- Optimization for ocrpt-docs: Only look for index under book
...
<xsl:template name="chunk-all-sections">
  ...
  <!-- Optimization for ocrpt-docs: Since we set a fixed $chunk.section.depth,
...
<xsl:template name="href.target">
  ...
  <!-- Optimization for ocrpt-docs: Remove support for dbhtml processing
       instruction here -->
...
<xsl:template name="html.head">
  ...
  <!-- Optimization for ocrpt-docs: Cut out a bunch of things we don't need

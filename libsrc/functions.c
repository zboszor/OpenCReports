...
static void ocrpt_abs(opencreport *o, ocrpt_expr *e) {
	if (e->n_ops == 1 && (e->ops[0]->type == OCRPT_EXPR_NUMBER || (e->ops[0]->result[o->residx] && e->ops[0]->result[o->residx]->type == OCRPT_RESULT_NUMBER))) {
...
static void ocrpt_add(opencreport *o, ocrpt_expr *e) {
	...
	nstr = 0;
	for (i = 0; i < e->n_ops; i++) {
		if (e->ops[0]->type == OCRPT_EXPR_NUMBER || e->ops[0]->result[o->residx]->type == OCRPT_RESULT_NUMBER)
			nnum++;
		if (e->ops[0]->type == OCRPT_EXPR_STRING || e->ops[0]->result[o->residx]->type == OCRPT_RESULT_STRING)
			nstr++;
	}
...
static void ocrpt_sub(opencreport *o, ocrpt_expr *e) {
	...
	nnum = 0;
	for (i = 0; i < e->n_ops; i++) {
		if (e->ops[0]->type == OCRPT_EXPR_NUMBER || (e->ops[0]->result[o->residx] && e->ops[0]->result[o->residx]->type == OCRPT_RESULT_NUMBER))
			nnum++;
	}
...
static void ocrpt_eq(opencreport *o, ocrpt_expr *e) {
	...
	if (!e->ops[0]->result[o->residx]) {
		ocrpt_expr_make_error_result(o, e, "first operand has no result");
		return;
	}

	if (!e->ops[1]->result[o->residx]) {
		ocrpt_expr_make_error_result(o, e, "second operand has no result");
		return;
	}

...
static void ocrpt_parse_query_node(opencreport *o, xmlTextReaderPtr reader) {
	...
	if (ds) {
		if (ocrpt_datasource_is_sql(ds))
			...
			ocrpt_query_discover_data(NULL, NULL, NULL, NULL, coltypes_s, &coltypesptr, &ct_cols_i, &free_types);
			...
			q = ocrpt_query_add_symbolic_data(ds, name_s, value_s, rows_i, cols_i, coltypes_s, coltypes_s ? cols_i : 0);
	}

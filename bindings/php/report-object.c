...
PHP_METHOD(opencreport_report, variable_new) {
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "lss|s!s!b", &variable_type, &name, &name_len, &expr, &expr_len, &ignoreexpr, &ignoreexpr_len, &reset_on_break_name, &reset_on_break_name_len, &precalculate)== FAILURE)
		return;
#endif

	ocrpt_var *v = ocrpt_variable_new(pro->r, variable_type, ZSTR_VAL(name), ZSTR_VAL(expr), ignoreexpr ? ZSTR_VAL(ignoreexpr) : NULL, reset_on_break_name ? ZSTR_VAL(reset_on_break_name) : NULL, precalculate);
	if (!v)
		RETURN_NULL();
...
PHP_METHOD(opencreport_report, variable_new_full) {
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "ls|s!s!s!s!s!s!b", &result_type, &name, &name_len, &baseexpr, &baseexpr_len, &ignoreexpr, &ignoreexpr_len, &intermedexpr, &intermedexpr_len, &intermed2expr, &intermed2expr_len, &resultexpr, &resultexpr_len, &reset_on_break_name, &reset_on_break_name_len, &precalculate) == FAILURE)
		return;
#endif
...
PHP_METHOD(opencreport_report, set_fielddetail_overlay) {
	zval *object = getThis();
	php_opencreport_report_object *pro = Z_OPENCREPORT_REPORT_P(object);
#if PHP_VERSION_ID >= 70000
	zend_string *expr_string = NULL;
	...
	ocrpt_expr *e = ocrpt_report_set_fielddetail_overlay(pro->r, expr_string ? ZSTR_VAL(expr_string) : NULL);
	if (!e)
		RETURN_NULL();
	object_init_ex(return_value, opencreport_expr_ce);
	php_opencreport_expr_object *eo = Z_OPENCREPORT_EXPR_P(return_value);
...
PHP_METHOD(opencreport_report, get_fielddetail_overlay) {
	zval *object = getThis();
	php_opencreport_report_object *pro = Z_OPENCREPORT_REPORT_P(object);

	ZEND_PARSE_PARAMETERS_NONE();

	ocrpt_expr *e = ocrpt_report_get_fielddetail_overlay(pro->r);
	if (!e)
		RETURN_NULL();
	object_init_ex(return_value, opencreport_expr_ce);
	php_opencreport_expr_object *eo = Z_OPENCREPORT_EXPR_P(return_value);
...
ZEND_ARG_VARIADIC_TYPE_INFO(0, precalculate, _IS_BOOL, 1)
ZEND_END_ARG_INFO()
...
OCRPT_BEGIN_ARG_WITH_RETURN_OBJ_INFO_EX(arginfo_opencreport_report_set_fielddetail_overlay, 0, 1, OpenCReport\\Expr, 1)
ZEND_ARG_TYPE_INFO(0, expr_string, IS_STRING, 1)
ZEND_END_ARG_INFO()

OCRPT_BEGIN_ARG_WITH_RETURN_OBJ_INFO_EX(arginfo_opencreport_report_get_fielddetail_overlay, 0, 0, OpenCReport\\Expr, 1)
ZEND_END_ARG_INFO()
...
#define arginfo_opencreport_report_set_fielddetail_overlay NULL
#define arginfo_opencreport_report_get_fielddetail_overlay NULL
...
static const zend_function_entry opencreport_report_class_methods[] = {
	...
	PHP_ME(opencreport_report, set_fielddetail_overlay, arginfo_opencreport_report_set_fielddetail_overlay, ZEND_ACC_PUBLIC | ZEND_ACC_FINAL)
	PHP_ME(opencreport_report, get_fielddetail_overlay, arginfo_opencreport_report_get_fielddetail_overlay, ZEND_ACC_PUBLIC | ZEND_ACC_FINAL)

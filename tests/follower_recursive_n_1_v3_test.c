...
static void test_newrow_cb(opencreport *o, ocrpt_report *r, void *ptr) {
	...
	ocrpt_query_result *qr2 = ocrpt_query_get_result(rd->q2, &cols2);
	ocrpt_query_result *qr3 = ocrpt_query_get_result(rd->q3, &cols3);
...
int main(int argc, char **argv) {
	...
	if (!ocrpt_query_add_follower_n_to_1(rd.q2, rd.q3, match2)) {
	...
	ocrpt_set_output_format(o, OCRPT_OUTPUT_TXT);

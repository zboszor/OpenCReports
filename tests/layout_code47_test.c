...
int main(int argc, char **argv) {
	...
	out = ocrpt_layout_report_footer(r);

	hl = ocrpt_output_add_hline(out);
	ocrpt_hline_set_size(hl, "1");
	ocrpt_hline_set_color(hl, "'blue'");

	l = ocrpt_output_add_line(out);

	t = ocrpt_line_add_text(l);
	ocrpt_text_set_value_string(t, "This is the report footer.");
	...
	/* End of report footer / field header / details / footer */
